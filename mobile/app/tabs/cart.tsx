  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      Alert.alert('Order Placed! 🎉', `Your order #${res.data.order?._id?.slice(-6).toUpperCase() || 'XXXX'} has been placed successfully.`, [
        { text: 'Track Order', onPress: () => router.push('/tabs/orders' as any) },
    ...
    return (
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        <View style={styles.emptyState}>
          ...
          <TouchableOpacity style={styles.browseBtn} onPress={() => router.push('/tabs/menu' as any)}>
          ...
          {items.map((item) => (
            <View key={`${item.dishId}-${item.size}`} style={styles.cartItem}>
              ...
              <View style={styles.qtyControl}>
                <TouchableOpacity
                  style={styles.qtyBtn}
                  onPress={() => updateQuantity(item.dishId, item.quantity - 1, item.size)}
                >
